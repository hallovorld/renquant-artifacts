...
1. **In-tree whole-index audit — `verify_canonical_index_integrity()` (new
   ...
   the raw writer runs it as a **fail-closed precondition** before it appends
   (never launders a corrupt/tampered prior entry forward), and asserts the
   persisted index is append-only vs what was read (`_index_append_only_errors`).
...
2. **Cross-COMMIT append-only — `verify_index_transition(base, candidate)`
   ...
   worktree, pinned to the **FIXED** `github.event.pull_request.base.sha`
   recorded at the PR event (checkout uses `fetch-depth: 0` so that sha is
   reachable) — never a re-fetched moving base tip, which could grab a later
   `main` and cause non-reproducible false rejections. This is the machine
   enforcement behind "any commit is verifiable" — **not a human reading a
   diff.** (Armed and inert today: no `canonical_publications` store on main.)
...
3. **Producer allow-list, bound to record identity.** `CANONICAL_PRODUCERS` is
   ...
   (This is an allow-list + shape gate on a SELF-REPORTED field, NOT a proof of
   origin — see the authenticity limitation below.)

4. **Candidate → verified-publisher boundary — `CandidatePublication` +
   `verify_candidate_authorization()` + `promote_candidate_publication()`, with
   the raw writer GATED.** The raw live-store writer is now module-private
   (`_register_canonical_publication`); the public package exposes **no** direct
   registration path, so `promote_candidate_publication` is the ONLY public way
   to write the live store and the authorization gate cannot be bypassed. A
   candidate is inert staging data; promote refuses an unauthorized/forged
   candidate (producer not in `CANONICAL_PRODUCERS`, or a tampered/unparsable
   run-intent) **before the live store is touched** — the live INDEX is
   byte-unchanged and no record file leaks in.
...
**Known limitation — producer AUTHENTICITY is NOT established here (honest, no
overclaim).** The allow-list checks the candidate's SELF-REPORTED `producer`
field; the content-address binds that claim but does not prove WHO produced or
committed the candidate — an attacker who writes an allow-listed producer NAME
into a fabricated run-intent still passes. Real cryptographic proof of origin
(a producer signature / GitHub workflow-identity attestation + an immutable
intake reference) is a separate substantial mechanism tracked as **follow-up
renquant-artifacts#33**. This PR delivers the invariant + transition verifier +
pair-identity schema + promote-gating; it does **not** claim verified-publisher
completeness.
...
## Sequencing (open follow-ups — #31 and #33)
...
- **This PR (independent, now):** the registry append-only + content-addressed
  ...
  boundary with the raw writer gated, and the additive pair-identity schema.
- **Deferred to the AC4-seal producer hook (design §4/§6, part of #31):**
  POPULATING the pair-identity values (`bundle_id`/member digests only exist at
  seal). The admission adapter (orchestrator) is the separate independent piece.
  So this PR does **not** claim `Closes #31`; #31 stays open.
- **Deferred to renquant-artifacts#33:** cryptographic producer AUTHENTICITY
  (signature / workflow-identity attestation + immutable intake ref) — the
  self-reported-producer gap above. No `Closes` for it either.
- No `renquant-artifacts` pin advances on this PR.

## Tests — `tests/test_canonical_index_invariant.py` (35, independent)
...
rejected with the live store byte-unchanged; the authorization envelope. Write-
path gating (`test_no_public_direct_live_store_write_path`): the public package
exposes no raw registration writer; only `promote_candidate_publication`. CI
config (`TestCiAppendOnlyGuardConfig`): the guard step pins the FIXED
`base.sha`, never `FETCH_HEAD`/a moving `base.ref`, and requires `fetch-depth:
0`. Pair-identity (`TestPairIdentitySchema`): present-nullable field by default,
valid binding stored/resolved, malformed rejected, rebinding an existing entry
refused.

The `register_canonical_publication` rename to a module-private primitive is
absorbed by `test_experiment_registry.py` / `test_artifact_registry.py` via a
one-line white-box import alias (their registry-primitive/fixture calls are
unchanged in behaviour).

## Evidence

Artifacts suite: **345 passed** (310 at the `0b67302f` baseline + 35 new), via
...
CI append-only guard verified locally (append→exit 0, deletion→exit 1).
